...
int main(int argc, char const *argv[]) {
    ...
    std::cout << "Is in normalized range: " << (float)X::one() << " " << X::one().is_unitary_range() << std::endl;
    std::cout << "Is in normalized range: " << (float)y << " " << y.is_unitary_range() << std::endl;
    std::cout << "Is !negative " << y.isnegative() << std::endl;
    std::cout << "One Minus " << (float)y << "(hex " << std::hex << y.v << ") " << (float)(X::one()-y) << "(hex " << (X::one()-y).v <<  ") and fast " << (float)(y.one_minus_ur()) << " (hex " << (y.one_minus_ur()).v  <<  ")"<< std::endl;

...
struct softfloat32
{
	...
	explicit softfloat32(int64_t f): value(i64_to_f32(f)) {}
	explicit softfloat32(float f): value((float_to_f32(f))) {}
	softfloat32(double f) : value(f64_to_f32(double_to_f64(f))) {}
	...
	explicit operator double () const { return f64_to_double(f32_to_f64(value)); }
	...
	softfloat32 operator= (double a)  { return softfloat32(a); }
	softfloat32 operator= (float a)  { return softfloat32(a); }
	softfloat32 operator= (int32_t a)  { return softfloat32(a); }
...
namespace std
{
	softfloat32 abs(softfloat32 a) {
		return a>=softfloat32(0)? a:-a;
	}
